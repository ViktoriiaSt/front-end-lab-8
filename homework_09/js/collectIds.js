...
let collectIds = (movies) => {
    let filteredArray = getFilteredArray(movies, function(element) {
        return element.rating > 3.0;
    });
    let filteredArrayWithId = getTransformedArray(filteredArray, function(element) {
        return element.id;
    });
    return filteredArrayWithId;
}